...
智能院舍照護管理前端（React + TypeScript + Vite）。業務母本與缺口追蹤見 `docs/pdf/` 與下列文件。工程規範與部署／驗收閘門見專案根目錄 **`.cursorrules`**（**§3**）及 **`docs/business-logic.md`** §0；序號清單主檔「**運維與工程**」路徑彙列見 **`docs/pdf-sequenced-gap-checklist.md`**（與 **§0**／**§3** 對齊）。**全案收尾**見下方「專案收尾」小節（表內含 Gate A **`docs/evidence/gate-a-latest.md`**；檔內 **Next Command** 與 **`preflight:strict`** 並列；檔尾 blockquote 四行：**`gateALatestMarkdownFooterLines`**，見 **`docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment`** 下文 **`latest`** 段）；多數 **`gatea:evidence:*`** stdout **`gateAStandardCloseoutBlockquotes`** 兩行（**第一行**併主日誌 **Gate A／stdout** 歸檔 **`docs/pdf-sequenced-gap-checklist-revision-log-archive-gate-a-stdout-2026-05-09.md`**；見「**Gate A 終端頁尾（維護）**」）；人工 **`docs/gate-a-manual-evidence-checklist-2026-05-06.md`** 開首（strict-http／keep=1；**`docs/go-live-checklist.md`** §0.1），並與 **`docs/business-logic.md`** §0 **全案收尾執行** 併述 **README** 表前互鏈指引（含各檔 **對照**／**全案收尾母索引**），**`docs/pdf-sequenced-gap-checklist.md`** 開首「全案收尾執行與證據留痕」並讀。**`docs/phase*.md`**／**`stage*.md`** 分階與歷史交付文件開首 **對照** 均互鏈該主檔「**運維與工程**」列。
...
Gate C 簽核／PAT：**[gate-c-section7-signoff-wording-2026-05-15.md](docs/gate-c-section7-signoff-wording-2026-05-15.md)** · **[gate-c-pat-ops-runbook-2026-05-15.md](docs/gate-c-pat-ops-runbook-2026-05-15.md)** · 收尾 **[gate-c-closeout-checklist-2026-05-15.md](docs/gate-c-closeout-checklist-2026-05-15.md)** · UAT **`npm run gatec:uat:kickoff`** · Staging **[uat-staging-access-guide-2026-05-15.md](docs/uat/uat-staging-access-guide-2026-05-15.md)** · 本機 **`npm run gatec:staging:preview`**
...
建議順序：可選先跑 [project-completion-kickoff-checklist-2026-05.md](docs/project-completion-kickoff-checklist-2026-05.md)（約 30 分鐘）→ [docs/evidence/gate-a-latest.md](docs/evidence/gate-a-latest.md)（Gate A 自動證據；**Next Command** 與 **`preflight:strict`** 並列於該檔；檔尾 blockquote 四行：**`gateALatestMarkdownFooterLines`**，見 [docs/gate-a-status-2026-05-06-commands-appendix.md](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment) 下文 **`latest`** 段；細部見 [go-live-checklist.md](docs/go-live-checklist.md) 開首 Gate A 取證）→ [docs/gate-a-manual-evidence-checklist-2026-05-06.md](docs/gate-a-manual-evidence-checklist-2026-05-06.md)（strict-http／keep=1／僅 prune 後同步）→ 盤點 → 兩週計畫 → 追蹤板 → 日誌 → 證據索引。
...
| [docs/evidence/gate-a-latest.md](docs/evidence/gate-a-latest.md) | Gate A 證據固定入口（**Next Command** 與 **`preflight:strict`** 並列；**`npm run gatea:evidence:latest`**／**`refresh`** 更新；檔尾四行：**`gateALatestMarkdownFooterLines`**，見 [docs/gate-a-status-2026-05-06-commands-appendix.md](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment) 下文 **`latest`** 段）；人工與 strict-http／keep=1 見 [docs/gate-a-manual-evidence-checklist-2026-05-06.md](docs/gate-a-manual-evidence-checklist-2026-05-06.md) 開首 |
...
| [pdf-sequenced-gap-checklist-revision-log-archive-p5.md](docs/pdf-sequenced-gap-checklist-revision-log-archive-p5.md) | 修訂日誌歸檔（p5：2026-05-03 續列；與主日誌並讀） |
| [pdf-sequenced-gap-checklist-revision-log-archive-seq29-2026-05-09b.md](docs/pdf-sequenced-gap-checklist-revision-log-archive-seq29-2026-05-09b.md) | 修訂日誌歸檔（Seq 29：2026-05-09 互鏈批量列；與主日誌並讀） |
| [pdf-sequenced-gap-checklist-revision-log-archive-gate-a-stdout-2026-05-09.md](docs/pdf-sequenced-gap-checklist-revision-log-archive-gate-a-stdout-2026-05-09.md) | 修訂日誌歸檔（Gate A／stdout：2026-05-09 細列；與主日誌並讀） |
...
npm run test:e2e:smoke   # build:demo 後僅跑 e2e/smoke.spec.ts（較快；含 #system-settings 本機 group／Pdf16 清單區／政策清單收合展開／五區 aria-controls id 相異／審計展開收合與 hidden 等 Seq 29 P1 煙霧，見 docs/seq29-system-settings-pdf02-traceability.md 第 4 節與 docs/seq29-system-settings-pdf02-traceability-revision-log.md）
npm run test:e2e:system-settings-policy # build:demo 後 Seq 29 P1 系統設定 demo（無 Supabase；Pdf16／政策清單收合展開／五區 aria-controls id 相異／審計展開收合等；見 docs/uat/system-settings-policy-p1-uat-and-staging-2026-05-09.md 與 docs/seq29-system-settings-pdf02-traceability.md 第 4 節、docs/seq29-system-settings-pdf02-traceability-revision-log.md）
npm run test:e2e:all     # demo 煙霧 + 可選登入 E2E（無 E2E_AUTH_* 時登入段 skip）
npm run test:e2e:auth    # 僅可選登入 E2E（playwright.auth.config.ts；Seq 29 **#system-settings** **P2** 五 **h3** 見 `e2e/auth-login.system-settings-p2.spec.ts`，需 `VITE_SUPABASE_*` 與 `E2E_AUTH_TEAMLEAD_*` 或 `E2E_AUTH_ADMIN_*`）
...
npm run gatea:evidence:latest # 更新固定入口 docs/evidence/gate-a-latest.md（檔尾四行 gateALatestMarkdownFooterLines；定義見 docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment 下文 latest 段）
...
- **CI**：推上 GitHub 後由 [`.github/workflows/ci.yml`](.github/workflows/ci.yml) 執行（含 Playwright 快取；demo 建置為 **`npm run build:demo`**，避免與本機 `.env` 內嵌 Supabase 混用；續跑 **`perf:bundle:ci`** 與 **`e2e/list-section-collapse.spec.ts`**，與根目錄 **`npm run ci`** 步序一致）。檔首註解並列 **Seq 29【16】**（**`npm run ci`** 內 **`e2e/smoke`** 之 **`#system-settings`**、**UAT** **二之一**（段末 **工程維護互鏈**）；本機可另跑 **`npm run test:e2e:auth`** 驗 **P2** **`h3`**，見 [docs/seq29-system-settings-pdf02-traceability.md](docs/seq29-system-settings-pdf02-traceability.md) 開首）；維護對照 [docs/pdf03-cursorrules-alignment.md](docs/pdf03-cursorrules-alignment.md) §3 **文件入口**、[docs/seq29-system-settings-pdf02-traceability.md](docs/seq29-system-settings-pdf02-traceability.md) 第 4 節與 [seq29-system-settings-pdf02-traceability-revision-log.md](docs/seq29-system-settings-pdf02-traceability-revision-log.md)。
...
- **Gate A 取證助手**：證據固定入口 **`docs/evidence/gate-a-latest.md`**（**Next Command** 與 **`npm run gatea:evidence:preflight:strict`** 並列；檔尾 blockquote 四行：**`gateALatestMarkdownFooterLines`**，見 [docs/gate-a-status-2026-05-06-commands-appendix.md](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment) 下文 **`latest`** 段；與 [go-live-checklist.md](docs/go-live-checklist.md) **§0.1** 並讀）。可用 **`npm run gatea:evidence:list`** 列出所有 `gatea:evidence:*`（與 `package.json` 同步）；步驟見 [docs/gate-a-evidence-capture-2026-05-06.md](docs/gate-a-evidence-capture-2026-05-06.md)，人工與 **strict-http**／**--keep=1**／僅 prune 後同步見 [docs/gate-a-manual-evidence-checklist-2026-05-06.md](docs/gate-a-manual-evidence-checklist-2026-05-06.md) 開首；一鍵腳本 `npm run gatea:evidence:all`（預設先 `gatea:evidence:preflight:strict`，含 `doctor`/`report` 落檔等；`--no-preflight` 略過前置檢查；HTTP 嚴格狀態可用 `gatea:evidence:all:strict-http` 或 `--strict-http`）；收證後順便修剪舊快照、刷新 `gate-a-latest.md`、再 **`docs-sync`** 與 **`decision-sync`** 可用 **`npm run gatea:evidence:refresh`**（`--keep=N` 僅作用於 prune；另有 **`gatea:evidence:refresh:strict-http`**）；僅同步四份收尾 markdown 可用 `npm run gatea:evidence:docs-sync`；彙總 `npm run gatea:evidence:summary`，可否判定 `npm run gatea:evidence:ready`，下一步導引 `npm run gatea:evidence:next`；取證前診斷 `npm run gatea:evidence:preflight`（嚴格：`npm run gatea:evidence:preflight:strict`）；需直接當關卡可用 `npm run gatea:evidence:gate`（NOT_READY 即非 0）；缺口細項 `npm run gatea:evidence:doctor`；清理舊自動快照可用 `npm run gatea:evidence:prune`（預設 dry-run，`-- --apply` 才刪檔）。即時狀態：[docs/gate-a-status-2026-05-06.md](docs/gate-a-status-2026-05-06.md)（**§5** [收證指令／旗標速查](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment)）。
- **Gate A 終端頁尾（維護）**：多數指令 stdout 末兩行 blockquote 與 **`scripts/gate-a-markdown-footer.mjs`** 之 **`gateAStandardCloseoutBlockquotes`** 同源（**第一行**併主日誌 **Gate A／stdout** 歸檔 [pdf-sequenced-gap-checklist-revision-log-archive-gate-a-stdout-2026-05-09.md](docs/pdf-sequenced-gap-checklist-revision-log-archive-gate-a-stdout-2026-05-09.md)；**第二行**併 **人工／strict-http／keep=1**，見 [docs/gate-a-manual-evidence-checklist-2026-05-06.md](docs/gate-a-manual-evidence-checklist-2026-05-06.md) 開首與 [go-live-checklist.md](docs/go-live-checklist.md) §0.1）；**`http:auth`** 頁尾見 [收證附錄 inherit](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-inherit)；**`decision-mini`** 等其餘例外見該腳本檔首 **Export 契約**。
