...
- **Gate A 終端頁尾（維護）**：多數指令 stdout 末兩行 blockquote 與 **`scripts/gate-a-markdown-footer.mjs`** 之 **`gateAStandardCloseoutBlockquotes`** 同源；**`http:auth`** 頁尾見 [收證附錄 inherit](docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-inherit)；**`decision-mini`** 等其餘例外見該腳本檔首 **Export 契約**。
