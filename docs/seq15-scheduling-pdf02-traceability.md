...
- 變更 **`SchedulingWorkflowStepper`** 文案、**`buildSchedulingWorkflowUiModel`** 條件、**`useScheduling`** 儲存條件、**`buildWeeklyRosterStaffProfileLookup`**（**`weeklyRosterStaffLookup.ts`**）或 **`buildEngineConstraintsFromRulesAndUi`**／**`mapRulesToConstraints`**（**`schedulingHookHelpers.ts`**）時：同步本檔、**`pdf-sequenced-gap-checklist.md`** Seq 15、**`pdf-sequenced-gap-checklist-revision-log.md`**（**`docs/pdf03-cursorrules-alignment.md`** §4）。
