...
| `src/repositories/schedulingSessionRepository.test.ts` | **`InMemorySchedulingSessionRepository`** 淺拷貝；**`createSchedulingSessionRepository`**（無憑證→**InMemory**）；**`EdgeSchedulingSessionRepository.listSessions`**（**`fetch`**／**`buildEdgeInvokeHeaders`** mock） |
