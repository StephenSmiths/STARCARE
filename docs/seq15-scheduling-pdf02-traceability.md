...
| `src/features/scheduling/components/SchedulingDashboard.test.tsx` | **`SchedulingDashboard`**（**`useSchedulingDashboardViewModel`**／**`useAuth`**／週更 dry-run mock；主區塊與 **loadError**／**`runBlockedReason`** 透傳；**`happy-dom`**） |
