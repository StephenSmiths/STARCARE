...
| `src/services/schedulingConfigService.test.ts` | **`listSchedulingSessions`**（fallback **`skillMatched`**、主檔 **`mapServiceType`**、**`getStaffProfilesUnavailableLastList`**）；**`getRules`** 委派 |
| `src/services/schedulingLastBatchStorage.test.ts` | **`writeLastSchedulingBatchId`**／**`readLastSchedulingBatchId`**／**`clearLastSchedulingBatchId`**（**sessionStorage**；**happy-dom**） |
| `src/services/schedulingKpiService.test.ts` | **`calculateSchedulingKpis`**（**PDF 01** §4.1 族群分母；零院友／覆蓋率／衝突率／待補位率／人均指派） |
| `src/services/schedulingWindowSnapshotService.test.ts` | **`resolveSchedulingWindowSnapshot`**（無憑證本機；**`getCurrentBundle`** 錯誤與無 **`policyVersion`**；有版號時 **P1** 合併 **PDF 02【16】**） |
| `src/services/schedulingPersistenceService.test.ts` | **`SchedulingPersistenceService.saveScheduleAssignments`**（空指派／衝突／防重覆提交；**`saveBatch`**／**`writeLastSchedulingBatchId`**／本機或遠端審計） |
...
| `src/features/scheduling/services/schedulingReloadPageData.test.ts` | **`runSchedulingReloadPageData`**（成功／院友過濾／**`listSchedulingSessions`**／**`prefetchRules`** 錯誤與 **`prefetchRules(facilityId)`** 呼叫） |
...
| `src/features/scheduling/hooks/runSchedulingKpiHistoryRetryFlow.test.ts` | **`runSchedulingKpiHistoryRetryFlow`**（**`pendingClear`**／待上傳 **`reverse`** 順序／**`hydrateFromServer`** 與 **`toSchedulingKpiHistoryQuery`**；**clear**＋併發路徑） |
...
| 2026-05-09 | §4／§5：補 **`schedulingConfigService.test.ts`**（**`listSchedulingSessions`**／**`getRules`**）；§5 維護閉環補 **`schedulingConfigService.ts`**。 |
| 2026-05-09 | §4／§5：擴 **`schedulingReloadPageData.test.ts`**（時段／**`prefetchRules`** 錯誤與呼叫）；§5 維護閉環補 **`schedulingReloadPageData.ts`**。 |
| 2026-05-09 | §4／§5：補 **`schedulingLastBatchStorage.test.ts`**（**sessionStorage** batch_id）；§5 維護閉環補 **`schedulingLastBatchStorage.ts`**。 |
