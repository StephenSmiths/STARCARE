...
> **既有讀規則**：**`scheduling-rules-get`** 仍以 **`public.scheduling_rules`** 為啟用旗標等扁平權威；若 **`facility_scheduling_policy_versions`** 於「現在」有適用列，**`groupCapacityLimit`** 以 **`facility_policy_numeric_limits.group_participant_cap`**（與 **`scheduling-policy-current-get`** 之 **`numericLimits.groupParticipantCap`** 同源）覆寫（PRD §7 **B** 過渡）；**其餘扁平欄** 仍待與版本化政策完全合併或遷移（並更新 **`docs/adr-0001-scheduling-logic-placement.md`** 附註）。
