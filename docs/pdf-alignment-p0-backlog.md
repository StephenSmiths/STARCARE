...
**工程／驗收附錄**：本機全閘 **`npm run ci`**（**`README.md`**、`docs/feature-list.md` §8）；遠端部署 **`npm run ops:deploy:all`** 見 **`docs/supabase-deploy-runbook.md`** §2；審計正式庫抽測見 **`docs/go-live-checklist.md`** §8；PAT 與部署後自檢見 **`docs/security-token-rotation-checklist.md`**。院友／評估完成之 Edge 契約見 **`docs/residents-edge-function-contract.md`**、**`docs/assessment-completion-records-contract.md`**。
...
| 2026-05-03 | **工程／驗收附錄**補 **`residents-edge-function-contract`**、**`assessment-completion-records-contract`** 鏈結。 |
