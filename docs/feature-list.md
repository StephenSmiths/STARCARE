...
7. **CI**：`.github/workflows/ci.yml` 於 push（`main`／`master`／`develop`）／PR 跑 `lint`、**`npm run typecheck`**（`tsc -b --noEmit`）、`vitest`、**demo `npm run build`**（清空 `VITE_SUPABASE_*`）、`test:e2e`（`PW_PREVIEW_ONLY=1` 時 webServer 僅 `preview`；含 **`actions/cache`** 快取 `~/.cache/ms-playwright`、`npx playwright install --with-deps chromium`、`concurrency`）。本機一鍵 **`npm run ci`**（`lint`→`typecheck`→`test`→`test:e2e`，由 Playwright 內建 demo 建置）；或 **`npm run test:e2e:all`** 連跑可選登入（無密鑰時 skip）。  
