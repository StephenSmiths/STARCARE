...
*文件產生：依倉庫 `src/`、`supabase/functions/`、`docs/go-live-checklist.md`、`docs/supabase-deploy-runbook.md`（§2 遠端部署、§6 前端 CI）與 `docs/security-token-rotation-checklist.md`（憑證；**§D** 可選 **`npm run ci`**）整理；**`acceptance:*`** 與 **`npm run ci`** 對照另見 **`docs/phase4-day4-delivery-index.md`**、**`docs/phase5-day1-delivery-index.md`**；專案根 **`.cursorrules`** §3 見 **`docs/business-logic.md`** §0。若與實機行為不符，以程式與 DB 為準。*
