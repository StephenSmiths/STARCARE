...
3. **README**：已改為 STARCARE 專案入口（文件表含 **`pdf-alignment-p0-backlog`**、**`adr-0001-scheduling-logic-placement`**、**`residents-edge-function-contract`**、**`assessment-completion-records-contract`**、**`client-delivery-remediation-plan`**、**`phase4-day4-delivery-index`**／**`phase5-day1-delivery-index`**、**`pdf03-cursorrules-alignment`**、憑證清單（**§D** 可選 **`npm run ci`**）、runbook；常用指令區含 **CI**、**憑證與部署後自檢（§D）** 短文；範本長文已收斂為選讀連結。  
...
*文件產生：依倉庫 `src/`、`supabase/functions/`、`docs/go-live-checklist.md`、`docs/supabase-deploy-runbook.md`（§2 遠端部署、§6 前端 CI）與 `docs/security-token-rotation-checklist.md`（憑證；**§D** 可選 **`npm run ci`**）整理；**`acceptance:*`** 與 **`npm run ci`** 對照另見 **`docs/phase4-day4-delivery-index.md`**、**`docs/phase5-day1-delivery-index.md`**。若與實機行為不符，以程式與 DB 為準。*
