...
3. **README**：已改為 STARCARE 專案入口（文件表含 **`pdf-alignment-p0-backlog`**、**`adr-0001-scheduling-logic-placement`**、**`assessment-completion-records-contract`**、**`client-delivery-remediation-plan`**、**`pdf03-cursorrules-alignment`**、憑證清單、runbook；`npm run ci`／E2E、CI 連結）；範本長文已收斂為選讀連結。  
