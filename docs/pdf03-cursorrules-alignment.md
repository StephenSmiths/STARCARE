...
- 變更 **`scripts/gate-a-markdown-footer.mjs`**、**`scripts/gate-a-ref-sync-lib.mjs`**、**`scripts/gate-a-update-latest-pointer.mjs`** 或 **`gate-a-evidence-list`**／**`gate-a-evidence-summary`**／**doctor**／**report** 等 Markdown／marker 產出邏輯時：視需要 **`npm run gatea:evidence:docs-sync`** 驗證 **`project-completion-*`** marker 區，並於 **`docs/pdf-sequenced-gap-checklist-revision-log.md`** 補列（與上項 **Gate A 取證敘述** 並讀）。
