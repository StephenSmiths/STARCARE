...
- 變更 **Seq 29** 系統設定政策 P1（**`#system-settings`** 政策版本區、相關 **`scheduling-policy-*`** 或 demo E2E）時，檢視 **`e2e/system-settings-policy-p1-demo.spec.ts`**、**`npm run test:e2e:system-settings-policy`**、**`npm run test:e2e:smoke`**（**`hash #system-settings`**）、**`docs/feature-list.md`** §8 第 5 點與 **`docs/uat/system-settings-policy-p1-uat-and-staging-2026-05-09.md`**（**二之一**）、**`docs/supabase-deploy-runbook.md`**（§2 **Seq 29**／§6）、**`docs/go-live-checklist.md`** §5、**`docs/seq29-system-settings-pdf02-traceability.md`** §5、**`.github/workflows/ci.yml`** 檔首 **Seq 29【16】** 專段是否仍一致。  
- 變更 **`README.md`** 開頭／「專案收尾」表前互鏈段落／讀者指引（**對照**／**全案收尾母索引**）／文件表、**`business-logic.md`** §0 分階交付句、**`docs/business-logic-revision-log.md`**（**§8** 修訂表）、**`.github/workflows/ci.yml`** 文件入口註解（含 **Seq 29【16】** 專段；與 §3 **文件入口**／**Seq 29【16】政策 P1（demo E2E）** 項並讀），或 **`docs/phase*.md`**／**`stage*.md`** 開首 **對照** 模式時，視需要同步 **`.cursorrules`** §3、**`pdf-sequenced`** 主檔「**運維與工程**」列、**`feature-list.md`** §8 README 項與本節「**`.cursorrules`** §3 部署與驗收閘門」項並讀。  
