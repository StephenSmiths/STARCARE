...
- 變更 **`scripts/gate-a-markdown-footer.mjs`**（檔首 **Export 契約**／「未匯入本檔之腳本」註解；**`gateALatestMarkdownFooterLines`** 即 **`docs/evidence/gate-a-latest.md`** 檔尾固定入口＋全案收尾雙行＋stdout 頁尾）、**`scripts/gate-a-ref-sync-lib.mjs`**、**`scripts/gate-a-update-latest-pointer.mjs`** 或 **`gate-a-http-evidence`**／**`gate-a-evidence-prune`**／**`gate-a-sync-all-markdown`**（**`--suppress-closeout-footer`** 僅供 **`gate-a-evidence-all`** 內嵌呼叫，避免與 **`gate-a-evidence-summary`** 重複）／**`gate-a-sync-decision-draft`**／**各 `gate-a-sync-*` 單檔同步**／**`gate-a-evidence-list`**／**`gate-a-evidence-summary`**／**`gate-a-evidence-next`**／**`gate-a-evidence-preflight`**／**`gate-a-evidence-ready`**／**`gate-a-evidence-gate`**／**doctor**／**report** 等 Markdown／marker 產出邏輯時：視需要 **`npm run gatea:evidence:docs-sync`** 驗證 **`project-completion-*`** marker 區，並於 **`docs/pdf-sequenced-gap-checklist-revision-log.md`** 補列（與上項 **Gate A 取證敘述** 並讀）。
