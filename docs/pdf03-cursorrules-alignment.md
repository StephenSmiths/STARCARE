...
- 變更 **`README.md`** 開頭／「專案收尾」表前互鏈段落／讀者指引（**對照**／**全案收尾母索引**）／文件表、**`business-logic.md`** §0 分階交付句、**`docs/business-logic-revision-log.md`**（**§8** 修訂表）、**`.github/workflows/ci.yml`** 文件入口註解，或 **`docs/phase*.md`**／**`stage*.md`** 開首 **對照** 模式時，視需要同步 **`.cursorrules`** §3、**`pdf-sequenced`** 主檔「**運維與工程**」列、**`feature-list.md`** §8 README 項與本節上一項。  
