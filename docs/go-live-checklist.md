...
- [x] 以 **`teamlead`** 或 **`admin`** 登入：儀表 **`/#dashboard`** 首屏底部見 **全域審計摘要**（或等效區塊），列表可載入、無持續性錯誤提示。
- [x] 至少開啟一處含 **`AuditTrailPanel`** 的模組（例如 **`/#scheduling`**、**`/#residents`**、**`/#service-forms`**），見審計標題與列表區；與 **`e2e/auth-login.spec.ts`**／**`e2e/auth-login.staff-modules.spec.ts`** 預期一致。
- [x] 於 UI 執行 **1 次**會觸發 **`audit-trail-append`** 的操作（例如排班儲存、院友主檔更新、服務表單送審等，依當日上線範圍）；成功後重新整理該頁或返回儀表，新事件出現於審計列表或摘要（依產品行為）。
...
- [x] **RLS 抽測**：**`staff`** 帳號僅能見 **`actor_id`＝本人** 之事件；**`teamlead`／`admin`** 可見全院 **`is_deleted = false`** 列（與 **`supabase/migrations/20260502133000_audit_events.sql`** 註解一致）。證據：`gateA-d4-rls-staff-2026-05-06.png` 等（見 **`docs/gate-a-manual-evidence-checklist-2026-05-06.md`** §C）。
