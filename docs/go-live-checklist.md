...
> **對照**：運維、憑證與前端 **`npm run ci`** 等文件總覽見 **`docs/business-logic.md`** §0（**`.cursorrules`** §3「部署與驗收閘門」、**`README.md`** 文件表）；序號清單主檔「**運維與工程**」路徑彙列見 **`docs/pdf-sequenced-gap-checklist.md`**；序號主檔修訂日誌（主檔與 **`2026-05-01a`**／**`archive-p2`**／**`archive-p3`**／**`archive-p4`**／**`archive-p5`**／**`archive-seq29-2026-05-09b`** 歸檔）路徑見 **`README.md`**「專案收尾」文件表（六檔 **對照**／**全案收尾母索引** 併讀 **`docs/business-logic.md`** §0）；Gate A 自動證據固定入口 **`docs/evidence/gate-a-latest.md`**（檔內 **Next Command** 與 **`preflight:strict`** 並列；**`npm run gatea:evidence:preflight:strict`** 等；檔尾 blockquote 四行：**`gateALatestMarkdownFooterLines`**（**`docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment`** 下文 **`latest`** 段））；多數 **`gatea:evidence:*`** 終端 stdout 頁尾 **`scripts/gate-a-markdown-footer.mjs`**（**Export 契約**）；細目 **§0.1**）。

**全案收尾與證據留痕**：見 **`README.md`**「專案收尾」小節（**`docs/business-logic.md`** §0 **全案收尾執行** 併述 **README** 表前互鏈指引（含各檔 **對照**／**全案收尾母索引**）；**`docs/project-completion-*.md`**、主修訂日誌 **`docs/pdf-sequenced-gap-checklist-revision-log.md`** 及歸檔 **`docs/pdf-sequenced-gap-checklist-revision-log-2026-05-01a.md`**、**`docs/pdf-sequenced-gap-checklist-revision-log-archive-p2.md`**、**`docs/pdf-sequenced-gap-checklist-revision-log-archive-p3.md`**、**`docs/pdf-sequenced-gap-checklist-revision-log-archive-p4.md`**、**`docs/pdf-sequenced-gap-checklist-revision-log-archive-p5.md`**、**`docs/pdf-sequenced-gap-checklist-revision-log-archive-seq29-2026-05-09b.md`**）；快速啟動 **`docs/project-completion-kickoff-checklist-2026-05.md`**；完成度盤點 **`docs/project-completion-audit-2026-05-05.md`**；證據索引 **`docs/project-completion-evidence-index-2026-05.md`**；Gate A 固定入口 **`docs/evidence/gate-a-latest.md`**（檔內 **Next Command** 與 **`preflight:strict`** 並列；**`npm run gatea:evidence:list`**／**`npm run gatea:evidence:refresh`**／取證前嚴格檢查 **`npm run gatea:evidence:preflight:strict`**；檔尾 blockquote 四行：**`gateALatestMarkdownFooterLines`**（**`docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment`** 下文 **`latest`** 段））；多數 **`gatea:evidence:*`** 終端 stdout 頁尾 **`scripts/gate-a-markdown-footer.mjs`**（**Export 契約**）；細部 **§0.1**）。  
...
- 人工證據勾選表：**`docs/gate-a-manual-evidence-checklist-2026-05-06.md`**（開首：**HTTP 嚴格取證** 全流程、**`--keep=1`**、僅 prune 後 **`latest`／`docs-sync`／`decision-sync`** 並帶 **`--strict-http`**；與上兩檔互鏈）
...
- 一鍵：全流程 → 修剪舊快照 → 刷新上列入口 → 同步收尾 markdown／判定稿：**`npm run gatea:evidence:refresh`**；須 **HTTP 狀態嚴格**（401／403）且 **`gate-a-latest`**／收尾四檔自動引用區之 **HTTP 嚴格取證** 與當次一致時，用 **`npm run gatea:evidence:refresh:strict-http`**（或合併環境 **`GATEA_STRICT_HTTP=1`** 後 **`npm run gatea:evidence:refresh`**；見 **`docs/gate-a-status-2026-05-06.md`** **§5**、**`docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-latest-segment`**）
...
- 終端 stdout 末兩行 blockquote（**全案收尾**／**§5**）之維護：**`scripts/gate-a-markdown-footer.mjs`**（檔首 **Export 契約**；**`http:auth`** 見 **`docs/gate-a-status-2026-05-06-commands-appendix.md#gate-a-appendix-inherit`** **inherit** 說明）；**`README.md`**「Gate A 終端頁尾（維護）」列並讀。
