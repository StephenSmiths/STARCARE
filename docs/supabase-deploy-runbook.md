...
- **Seq 29 系統設定（無 Supabase bundle）**：專項 **`npm run test:e2e:system-settings-policy`**／**`npm run test:e2e:smoke`**（**`hash #system-settings`**）見 **`docs/uat/system-settings-policy-p1-uat-and-staging-2026-05-09.md`** **二之一**（段末 **工程維護互鏈**）；與 **`docs/go-live-checklist.md`** §5 併讀。已設 **`VITE_SUPABASE_*`** 之本機可增跑 **`npm run test:e2e:auth`** 內 **`e2e/auth-login.system-settings-p2.spec.ts`**（**P2** **`h3`**；**`E2E_AUTH_TEAMLEAD_*`** 優先或 **`E2E_AUTH_ADMIN_*`**；見 **`docs/seq29-system-settings-pdf02-traceability.md`** 開首）。
