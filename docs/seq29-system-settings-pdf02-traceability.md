...
| `mergeP1DraftIntoPolicyBundle.test.ts`（Vitest） | P2：**`policyFixedActivitiesHydrated`**／雲端列保留與草稿覆寫 |
| `p1FieldsFromPolicyBundle.test.ts`（Vitest） | hydrate：**`policyFixedActivities`**／**`policyFixedActivitiesHydrated`** |
