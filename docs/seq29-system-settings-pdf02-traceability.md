...
| `SystemSettingsHome.test.tsx`（Vitest） | 整合：**`useAuth`**／**`useAuditTrailList`** mock 下 **Pdf16** 兩大節／**政策**／**審計** **`section[aria-labelledby]`**；**排班時間／規則／資助復康** **`ListSectionPanel`** 之 **`section`** 嵌於對應 **Pdf16** 大節內，且預設 **收合**／**收合**／**展開** 鈕語意正確；本機 **`group`** **`aria-busy="false"`**、**儲存設定（本機）**、**`supabaseBrowserEnv`** mock 下無 Edge **Supabase** 說明可見 |
