...
| `schedulingPolicyRepository.edgeReadWrite.test.ts`、`schedulingPolicyRepository.listPolicyVersionSummaries.test.ts`（Vitest） | **`EdgeSchedulingPolicyRepository`**：**請先登入**（**getCurrent**／**validate**／**commit** 不呼叫 **`fetch`**）／**fetch** 連線包裝（**getCurrent**／**validate**／**commit**；與 **`schedulingKpiHistoryRepository`** Edge 訊息一致）；併既有 **current-get**／**versions-list**／**validate**／**commit** 路徑 |
