...
| `e2e/smoke.spec.ts` | 各 hash 模組與審計標題；**`#staff-import`** 審計 **`aria-controls`**／**`hidden`**；**`#system-settings`**：**本機設定（瀏覽器儲存）** **`group`**、**政策版本** **`ListSectionPanel`** 預設展開（**`aria-controls`**、無 **`hidden`**）、**政策版本（P1）**、**儲存設定（本機）**、無 Edge 說明 |
| `e2e/system-settings-policy-p1-demo.spec.ts` | **`npm run test:e2e:system-settings-policy`**：本機區 **`group`**／**`aria-busy`**；**智能排班設定**／**復康服務基本設定**；**排班時間設定**（於 **Pdf16** 大節內篩 **`ListSectionPanel`**、**`h3`**、預設展開、**`aria-controls`**）與 **資助復康** **`ListSectionPanel`** **`aria-controls`**／**`hidden`**／展開後 **SC** 文案；政策版本（P1）／**審計** **`section[aria-labelledby]`** 與對應標題 **id**、無 Edge 本機說明 |
