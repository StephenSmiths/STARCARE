...
| `SystemSettingsSubsidizedTiersCard.test.tsx`（Vitest） | P2 資助三列：**載入預設** 委派 **`setField('policySubsidizedTiers', …)`** 與 **`policySubsidizedTiersHydrated`** |
| `SystemSettingsPassOrderCard.test.tsx`（Vitest） | P2 Pass 次序：**下移** 委派 **`setField('policySubsidizedPassOrder', …)`** 與 **`policySubsidizedPassOrderHydrated`** |
| `mergeP1DraftIntoPolicyBundle.test.ts`（Vitest） | P2：**`policyFixedActivitiesHydrated`**／雲端列保留與草稿覆寫；**`policySubsidizedPassOrderHydrated`**／Pass 次序；**`policySubsidizedTiersHydrated`**／資助三列 |
| `p1FieldsFromPolicyBundle.test.ts`（Vitest） | hydrate：**`policyFixedActivities`**／**`policyFixedActivitiesHydrated`**；**`policySubsidizedPassOrder`**；**`policySubsidizedTiers`** |
...
| 2026-05-09 | **P2 切片（固定活動）**：**`policyFixedActivities`**／**`policyFixedActivitiesHydrated`**、**`SystemSettingsFixedActivitiesCard`**、**`mergeP1DraftIntoPolicyBundle`**／**`p1FieldsFromPolicyBundle`**、**`policyFixedActivityDraft`**、**`systemSettingsValidation`**、**`systemSettingsRepository`**；**§1**／**§4**；Vitest **`mergeP1DraftIntoPolicyBundle.test`**、**`p1FieldsFromPolicyBundle.test`**、**`SystemSettingsFixedActivitiesCard.test`**。 |
| 2026-05-09 | **P2 切片（資助 Pass 次序）**：**`policySubsidizedPassOrder`**／**`policySubsidizedPassOrderHydrated`**、**`policyPassOrderDraft`**、**`SystemSettingsPassOrderCard`**、**`mergeP1DraftIntoPolicyBundle`**／**`p1FieldsFromPolicyBundle`**、**`schedulingPolicyTypes`** **`POLICY_SUBSIDIZED_FUNDING_TIERS`**；**§1**／**§4**；Vitest **`SystemSettingsPassOrderCard.test`**、**`merge`／`p1Fields`／`systemSettingsValidation`** 擴充。 |
| 2026-05-09 | **P2 切片（資助復康三列）**：**`policySubsidizedTiers`**／**`policySubsidizedTiersHydrated`**、**`PolicySubsidizedTierRow`**、**`policySubsidizedTierDraft`**、**`SystemSettingsSubsidizedTiersCard`**、**`mergeP1DraftIntoPolicyBundle`**／**`p1FieldsFromPolicyBundle`**、**`systemSettingsValidation`**、**`systemSettingsRepository`**；**§1**／**§4**；Vitest **`SystemSettingsSubsidizedTiersCard.test`**、**`merge`／`p1Fields`／`systemSettingsValidation`** 擴充。 |
