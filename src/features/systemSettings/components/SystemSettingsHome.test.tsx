...
describe('SystemSettingsHome', () => {
  it('Pdf16 大節、清單小節、政策區、審計與本機儲存語意可見', () => {
    ...
    expect(within(schedulePanel as HTMLElement).getByRole('button', { name: '收合' })).toBeTruthy()
    expect(within(rulesPanel as HTMLElement).getByRole('button', { name: '收合' })).toBeTruthy()
    expect(within(subsidizedPanel as HTMLElement).getByRole('button', { name: '展開' })).toBeTruthy()
