...
describe('SystemSettingsHome', () => {
  it('Pdf16 大節、政策區與本機儲存語意可見', () => {
    render(<SystemSettingsHome />)
    expect(screen.getByRole('heading', { name: '智能排班設定', exact: true })).toBeTruthy()
    expect(screen.getByRole('heading', { name: '復康服務基本設定', exact: true })).toBeTruthy()
    expect(screen.getByRole('group', { name: '本機設定（瀏覽器儲存）' })).toBeTruthy()
    expect(screen.getByRole('heading', { name: '政策版本（雲端提交）（P1）', exact: true })).toBeTruthy()
