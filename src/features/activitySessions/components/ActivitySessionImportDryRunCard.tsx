...
export type ActivitySessionImportDryRunCardProps = {
  canMaintainSessions: boolean
  actorId: string
  isBusy: boolean
  errorMessage: string
  parseErrors: Array<{ rowIndex: number; message: string }>
  result: ActivitySessionImportValidationResult | null
  commitMessage: string
  lastRunSummary: ImportRunSummary | null
  runHistory: ImportRunSummary[]
  validateCsvText: (csvText: string) => Promise<void>
  commitValidatedRows: (actorId: string) => Promise<void>
}

/** 活動時段 CSV：預檢／確認匯入與列表（PDF 02【3】閉環占位） */
export const ActivitySessionImportDryRunCard = ({
  canMaintainSessions,
  actorId,
  isBusy,
  errorMessage,
  ...
  validateCsvText,
  commitValidatedRows,
}: ActivitySessionImportDryRunCardProps) => {
          ...
          <>
            ...
            {result ? (
              <div className={uiTokens.residentDryRunResultShell}>
                <p>
                  預檢結果：總數 {result.summary.total}，可匯入 {result.summary.valid}，錯誤{' '}
                ...
                {visibleErrors.length > 0 ? (
                  <>
                    ...
                    {hasHiddenErrors ? (
                      <button
                        type="button"
                        className={uiTokens.btnCompactMt2}
