...
import {
  ...
  STAFF_PROFILE_SCOPE_OPTIONS,
  type StaffProfileRoleTypeOption,
} from '../utils/staffProfileEditConstants'

export type StaffProfileEditFormProps = {
...
export const StaffProfileEditForm = ({ row, actorId, onClose, onSaved }: StaffProfileEditFormProps) => {
  const [name, setName] = useState(row.staffName)
  const [roleType, setRoleType] = useState<StaffProfileRoleTypeOption>(row.roleType ?? 'PT')
        ...
        <div className={uiTokens.stackVerticalMt3}>
          ...
          <label className={uiTokens.formFieldStack}>
            ...
            <select
              ...
              value={roleType}
              onChange={(e) => setRoleType(e.target.value as StaffProfileRoleTypeOption)}
            >
              {STAFF_PROFILE_ROLE_OPTIONS.map((r) => (
          ...
          <label className={uiTokens.formFieldStack}>
            ...
            >
              {STAFF_PROFILE_SCOPE_OPTIONS.map((s) => (
